@echo off
REM ──────────────────────────────────────────────────────────────────
REM  Krexion — RE-TUNE for current hardware
REM
...
REM    4. Restart the Krexion stack with the new tuning
REM
REM  Useful when:
REM    - You added more RAM
REM    - You moved Krexion to a different / faster PC
...
echo.
echo  Step 4 / 4 -- Restarting Krexion with the new profile...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    ". '%~dp0scripts\detect-hardware.ps1'; ^
     $p = Get-KrexionProfile; ^
     Push-Location '%~dp0'; ^
