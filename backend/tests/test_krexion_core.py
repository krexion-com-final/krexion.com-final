"""
Krexion backend core smoke tests.

Coverage (per review_request):
  - GET  /api/diagnostics/health
  - POST /api/admin/login           (admin@krexion.local / admin123)
...
def test_user(session):
    ...
    email = f"TEST_{unique}@krexion.local"
    password = "Test1234!"
...
class TestLinks:
    ...
    def test_create_link_then_list(self, session, test_user):
        payload = {
            "offer_url": "https://example.com/offer?aff=TEST_krexion",
            "status": "active",
