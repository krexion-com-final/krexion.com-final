...
def _headers() -> dict:
    return {
        ...
        "User-Agent": "Krexion-Local/1.0.21",
    }
...
async def _execute_job_locally(job: dict, jwt_token: str | None = None) -> dict:
    ...
    if feature not in feature_routes:
        ...
        if isinstance(payload, dict) and payload.get("method") and payload.get("path"):
            ...
            # v1.0.21: faithful raw-body + content-type + auth replay so
            ...
            try:
                ...
                try:
                    ...
                    # v1.0.21: when response isn't JSON (e.g.
...
def start_if_local(main_db, get_db_for_user) -> bool:
    ...
    if not CLOUD_URL:
        ...
        return False
    # v1.0.21: kill the legacy PowerShell `KrexionBridge` scheduled task
    ...
    # v1.0.21 logs. Deleting the task on startup is idempotent
