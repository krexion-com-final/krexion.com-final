@echo off
REM ============================================================
REM  RealFlow QUICK FIX -- For users who got "Installation failed"
REM
REM  This single .bat file:
REM  1. Cleans up any stuck/partial install at C:\realflow
...
echo.
echo ============================================================
echo   RealFlow QUICK FIX + INSTALL
echo ============================================================
echo.
echo   This will:
echo   1. Clean any partial install at C:\realflow
echo   2. Download the latest installer from GitHub
echo   3. Install RealFlow fresh
echo.
...
REM Step 1 -- Hard cleanup C:\realflow
echo Step 1/3 -- Cleaning up any partial install...

if exist "C:\realflow" (
    echo   Found old install at C:\realflow, removing...
    REM Try to stop docker compose first
    pushd "C:\realflow" 2>nul
    docker compose down 2>nul
    popd 2>nul

    REM Take ownership + grant full permissions, then remove
    takeown /F "C:\realflow" /R /D Y >nul 2>&1
    icacls "C:\realflow" /grant "Administrators:F" /T /C /Q >nul 2>&1
    rmdir /s /q "C:\realflow" 2>nul
    ...
    if exist "C:\realflow" (
        echo.
        echo   ERROR: Could not delete C:\realflow
        echo.
        echo   FIX:
        echo     1. Close all File Explorer windows showing C:\realflow
...
set "DOWNLOAD_DIR=%TEMP%\realflow-installer"
...
echo   Downloading RealFlow-EASY-INSTALL.ps1...
powershell -NoProfile -Command "try { Invoke-WebRequest -UseBasicParsing -Uri '%BASE%/RealFlow-EASY-INSTALL.ps1' -OutFile '%DOWNLOAD_DIR%\RealFlow-EASY-INSTALL.ps1' -TimeoutSec 60 } catch { Write-Host '  Download failed:' $_.Exception.Message; exit 1 }"
if errorlevel 1 (
    echo.
...
echo   Downloading RealFlow-EASY-INSTALL.bat...
powershell -NoProfile -Command "try { Invoke-WebRequest -UseBasicParsing -Uri '%BASE%/RealFlow-EASY-INSTALL.bat' -OutFile '%DOWNLOAD_DIR%\RealFlow-EASY-INSTALL.bat' -TimeoutSec 30 } catch { exit 1 }"
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%DOWNLOAD_DIR%\RealFlow-EASY-INSTALL.ps1"
set "INSTALL_EXIT=%errorlevel%"

echo.
if "%INSTALL_EXIT%" == "0" (
    echo ============================================================
    echo   RealFlow installed successfully!
    echo ============================================================
    echo.
    echo   Open: http://localhost:3000
    echo   Admin password is shown in the installer log + C:\realflow\.env
    echo.
) else (
    echo ============================================================
