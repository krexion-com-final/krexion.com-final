...
:: ============================================================
::  Krexion - Secure One-Click Installer  (v2.1 - Dynabook)
::
::  Security layers:
...
curl.exe -s -o "%TEMP%\rf_body.txt" -w "%%{http_code}" -H "Authorization: Bearer %GH_PAT%" -H "Accept: application/vnd.github.v3.raw" -H "User-Agent: Krexion-Installer" --max-time 15 "%STATUS_API%" >"%TEMP%\rf_code.txt" 2>nul
...
curl.exe -s -o NUL -w "%%{http_code}" -H "Authorization: Bearer %GH_PAT%" -H "User-Agent: Krexion-Installer" --max-time 15 "https://api.github.com/repos/%GH_OWNER%/%GH_REPO%" >"%TEMP%\rf_code.txt" 2>nul
...
if exist "%INSTALL_DIR%\LOCAL-START.bat" (
    color 0E
    echo  [INFO] Krexion already installed - launch kar raha hoon...
    timeout /t 2 /nobreak >nul
    goto :LAUNCH
)
...
if not exist "%TEMP%\krexion-install" mkdir "%TEMP%\krexion-install"
set "ZIP_PATH=%TEMP%\krexion-install\krexion.zip"
if exist "%ZIP_PATH%" del /Q "%ZIP_PATH%" >nul 2>&1

powershell -NoProfile -Command "try { [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; $h=@{ 'Authorization'='Bearer %GH_PAT%'; 'User-Agent'='Krexion-Installer' }; Invoke-WebRequest -Uri '%ZIP_URL%' -OutFile '%ZIP_PATH%' -Headers $h -UseBasicParsing; exit 0 } catch { Write-Host $_.Exception.Message; exit 1 }"

if not exist "%ZIP_PATH%" (
    echo  [INFO] PowerShell se download fail. Curl se retry...
    curl.exe -sL -H "Authorization: Bearer %GH_PAT%" -H "User-Agent: Krexion-Installer" -o "%ZIP_PATH%" "%ZIP_URL%"
)
...
if exist "%TEMP%\krexion-install\extracted" rmdir /S /Q "%TEMP%\krexion-install\extracted" >nul 2>&1
powershell -NoProfile -Command "Expand-Archive -Path '%ZIP_PATH%' -DestinationPath '%TEMP%\krexion-install\extracted' -Force"

if not exist "%TEMP%\krexion-install\extracted\%EXTRACTED_FOLDER%" (
    color 0C
    echo  [X] Extract fail. Expected folder: %EXTRACTED_FOLDER%
    dir "%TEMP%\krexion-install\extracted"
    pause
    exit /b 1
)

if exist "%INSTALL_DIR%" rmdir /S /Q "%INSTALL_DIR%" >nul 2>&1
move "%TEMP%\krexion-install\extracted\%EXTRACTED_FOLDER%" "%INSTALL_DIR%" >nul
if errorlevel 1 (
    color 0C
...
rmdir /S /Q "%TEMP%\krexion-install" >nul 2>&1
...
color 0A
echo  +==========================================================================+
echo  ^|                    Krexion Successfully Deployed!                       ^|
