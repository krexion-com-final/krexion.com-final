...
REM ╔══════════════════════════════════════════════════════════════════╗
REM ║                                                                  ║
REM ║         KREXION — ONE-CLICK INSTALLER (Windows)                 ║
REM ║                                                                  ║
...
REM ║   4. Clone the Krexion repo to C:\krexion                      ║
...
if %errorLevel% neq 0 (
    echo.
    echo  Krexion installer needs Administrator privileges.
...
title Krexion One-Click Installer
color 0B
...
echo  Target install folder: C:\krexion
echo.
...
if exist "%~dp0KREXION-DEPLOY.ps1" (
    echo.
    echo  Found local KREXION-DEPLOY.ps1 — running it...
    echo.
    powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0KREXION-DEPLOY.ps1"
) else (
    echo.
    ...
    powershell -NoProfile -ExecutionPolicy Bypass -Command ^
      "Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force; ^
       iwr -UseBasicParsing https://raw.githubusercontent.com/ronaldsexedwards40-glitch/dynabook/main/KREXION-DEPLOY.ps1 -OutFile $env:TEMP\KREXION-DEPLOY.ps1; ^
       & $env:TEMP\KREXION-DEPLOY.ps1"
)
...
if %EXITCODE% equ 0 (
    color 0A
    echo  ✓ Krexion installation completed successfully.
    echo.
    echo  Open in browser:   http://localhost:8001/docs   (API)
    echo  Frontend:          deploy frontend separately on Vercel
    echo                     OR run yarn start inside C:\krexion\frontend
    echo.
    echo  Admin password was printed above. Save it now.
    echo.
    echo  Daily commands ^(run from C:\krexion^):
    echo      LOCAL-START.bat       — start
    echo      LOCAL-STOP.bat        — stop
    echo      KREXION-LOGS.bat     — live logs
    echo      KREXION-UPDATE.bat   — pull ^& rebuild
) else (
    color 0C
    ...
    echo   - For deeper help, see  DEPLOY-README-URDU.md  in C:\krexion
)
echo  ─────────────────────────────────────────────────────────────
echo.
pause
