...
────────────────────────────
  ...
  ✓  Installs Docker Desktop  (downloads ~520 MB if not present)
  ✓  Installs Git  (downloads ~50 MB if not present)
  ✓  Configures WSL2 memory based on your RAM
     (8 GB PC → 5 GB cap so Windows stays smooth)
  ✓  Downloads the RealFlow code from GitHub
  ✓  Generates a strong random admin password
  ✓  Builds and starts the app (FastAPI + MongoDB + React)
  ✓  Auto-detects 8 GB RAM and switches to Low-RAM profile
  ✓  Creates a "RealFlow" shortcut on your Desktop
