# ============================================================
# Krexion ULTIMATE INSTALLER - Auto-recovery, Bulletproof
# ============================================================
...
Start-Transcript -Path "$env:TEMP\krexion-transcript.log" -Append -ErrorAction SilentlyContinue
...
if ($totalRam -lt 3.5) {
    Write-Warn "Only $totalRam GB RAM detected. Krexion may run slow."
...
if ($needsReboot -and -not (Test-Path $RESUME_MARKER)) {
    ...
    try {
        $taskName = "KrexionResumeInstall"
        $scriptPath = $MyInvocation.MyCommand.Path
        if (-not $scriptPath) { $scriptPath = "$PSScriptRoot\Krexion-ULTIMATE-INSTALL.ps1" }
...
if (Test-Path $RESUME_MARKER) {
    ...
    Unregister-ScheduledTask -TaskName "KrexionResumeInstall" -Confirm:$false -ErrorAction SilentlyContinue
...
# ============================================================
# STEP 7: Download Krexion + Setup
# ============================================================
Write-Step "STEP 7/8: Downloading Krexion Code"
...
$zipPath = "$env:TEMP\krexion-main.zip"
$extractDir = "$env:TEMP\krexion-extract"
Write-Info "Downloading Krexion (~50 MB)..."
try {
    if (Test-Path $zipPath) { Remove-Item $zipPath -Force }
    if (Test-Path $extractDir) { Remove-Item $extractDir -Recurse -Force }
    Invoke-WebRequest -Uri $REPO_ZIP_URL -OutFile $zipPath -UseBasicParsing -TimeoutSec 300
    Write-Ok "Downloaded Krexion ZIP"
} catch {
...
# ============================================================
# STEP 8: Build + Start Krexion
# ============================================================
Write-Step "STEP 8/8: Building and Starting Krexion"
...
Write-Info "Waiting for Krexion to be ready (up to 2 minutes)..."
$elapsed = 0
$ready = $false
while ($elapsed -lt 120) {
    Start-Sleep -Seconds 5
...
if (-not $ready) {
    Write-Warn "Krexion didn't respond on port 3000 within 2 minutes."
...
Write-Host ("=" * 70) -ForegroundColor Green
Write-Host ""
Write-Host "  Krexion is now running on your PC!" -ForegroundColor White
Write-Host ""
...
Write-Host "    Update:  double-click KREXION-UPDATE.bat in $INSTALL_DIR" -ForegroundColor White
...
$credsFile = "$env:USERPROFILE\Desktop\Krexion-Credentials.txt"
$credsContent = @"
=================================================
  Krexion - Admin Credentials
=================================================
...
Write-Ok "Credentials backup saved to Desktop: Krexion-Credentials.txt"

# Create Desktop shortcut
try {
    $WshShell = New-Object -ComObject WScript.Shell
    $Shortcut = $WshShell.CreateShortcut("$env:USERPROFILE\Desktop\Krexion.url")
...
# Open browser
Write-Host "  Opening Krexion in your browser..." -ForegroundColor Cyan
