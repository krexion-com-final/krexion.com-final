"""
Krexion CPI — Local iOS Proxy Gateway
...
Workaround used here (zero touch after one-time setup):
  ...
  • This mitmproxy add-on then forwards each iPhone request through a
    ...
    populated live by the Krexion CPI Worker (orchestrator) per-install.
...
The worker writes to:
  /tmp/krexion-cpi-ios-proxy-pool.json     (Linux/macOS)
  C:\\krexion\\krexion-cpi-worker\\ios-proxy-pool.json  (Windows)
...
POOL_PATHS = [
    ...
    Path(os.path.expanduser("~/.krexion-cpi-ios-proxy-pool.json")),
]
