...
if %errorLevel% neq 0 (
    echo.
    echo  Krexion Setup needs Administrator privileges.
...
if %errorLevel% neq 0 (
    ...
    echo         Krexion Setup requires Windows 10 or 11.
    echo.
    pause
    exit /b 1
)
...
if not exist "%~dp0setup-engine.ps1" (
    ...
    echo         %~dp0
    echo.
    echo  Make sure you extracted the entire Krexion-Setup folder,
...
echo.
echo  Launching Krexion Setup wizard...
...
if %RC% neq 0 (
    ...
    echo       real-time protection for the Krexion-Setup folder.
